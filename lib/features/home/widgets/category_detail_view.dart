...
//     try {
//       final results = await SortService.sortByDistance(
//           widget.categoryId, position.latitude, position.longitude);
//       setState(() {
//         _filteredRestaurants = results;
//       });
//     } catch (e) {
...
class _CategoryDetailPageState extends State<CategoryDetailPage> {
  ...
  void _filterByDistance(String order) async {
    Position position = await Geolocator.getCurrentPosition(
        desiredAccuracy: LocationAccuracy.high);
    print(
        'User Position: ${position.latitude}, ${position.longitude}'); // Debug print
    try {
      final results = await SortService.sortByDistance(
          widget.categoryId, position.latitude, position.longitude, order);
      print('Filtered Results by Distance: $results'); // Debug print
      setState(() {
        _filteredRestaurants = results;
      });
    } catch (e) {
              ...
              Row(
                children: [
                  ...
                  ElevatedButton(
                    onPressed: () {
                      Navigator.pop(context);
                      _filterByDistance('desc'); // Longest
                    },
                ...
                Row(
                  ...
                  children: [
                    InkWell(
                      onTap: () {
                        Navigator.pushNamed(
                            context, '/map'); // Navigate to map page
                      },
                      child: const Row(
                        children: [
                          Icon(Icons.location_on, color: Colors.black),
